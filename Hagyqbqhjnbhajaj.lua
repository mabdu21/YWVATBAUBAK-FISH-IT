-- Powered by dyumra | v449 (Reworked)
-- =========================
local version = "Rework"
local ver     = "v014.18"
-- =========================
-- CHANGELOG v014.18
-- [Fixed]   ESP Survivor/Killer ไม่แสดง Name/Distance/HP (setObjectLabels ไม่ถูกเรียกสำหรับ player)
-- [Fixed]   ESP Killer detection (เช็ค Weapon ทุก descendant ไม่ใช่แค่ direct child)
-- [Fixed]   Full Bright / No Fog ไม่ restore ตอน load (เพิ่ม restore block)
-- [Fixed]   espObjects leak memory (pcall + parent check ก่อน iterate)
-- [Fixed]   ShowName / ShowDistance / ShowHP ไม่ทำงานกับ player ESP
-- [Improved] ESP player label update loop แยกออกจาก world object loop
-- [Improved] isKillerChar() helper แทนการเช็ค Weapon ซ้ำๆ
-- =========================
...
Info:Divider()
Info:Paragraph({
    Title = "Update: 05/25/2026 | CL: " .. ver,
    Desc  = "• [ Fixed ] ESP Survivor/Killer not sync Name/Distance/HP\n• [ Fixed ] ESP Killer detection (weapon deep search)\n• [ Fixed ] Full Bright / No Fog not restoring on load\n• [ Fixed ] Esp objects memory leak\n• [ Fixed ] ShowName/ShowDistance/ShowHP not working\n• [ Improved ] ESP player label loop separated from world objects\n• [ Improved ] Get killer system helper function\n• [ Rework ] Auto Parry v4 (velocity spike pre-detect)\n• [ Rework ] Generator System (smart cancel, position-based)",
})
Info:Divider()

-- =====================================================================================
--  AUTO PARRY SYSTEM v4
-- =====================================================================================
local UIS_AP  = game:GetService("UserInputService")
local Vim_AP  = game:GetService("VirtualInputManager")
local LP_AP   = Players.LocalPlayer
...
local COOLDOWN_COLOR = Color3.fromRGB(77, 77, 77)
...
local ANIM_HITFRAME = {
    ...
    ["rbxassetid://118907603246885"] = { preDelay=0.00, hitAt=0.20 },
    ["rbxassetid://78432063483146"]  = { preDelay=0.00, hitAt=0.20 },
    ["rbxassetid://113255068724446"] = { preDelay=0.00, hitAt=0.13 },
    ["rbxassetid://74968262036854"]  = { preDelay=0.00, hitAt=0.13 },
    ["rbxassetid://129784271201071"] = { preDelay=0.00, hitAt=0.13 },
    ["rbxassetid://132817836308238"] = { preDelay=0.00, hitAt=0.20 },
    ["rbxassetid://112166042383605"] = { preDelay=0.00, hitAt=0.20 },
    ["rbxassetid://122812055447896"] = { preDelay=0.00, hitAt=0.20 },
    ["rbxassetid://117042998468241"] = { preDelay=0.00, hitAt=0.20 },
    ["rbxassetid://133963973694098"] = { preDelay=0.00, hitAt=0.20 },
}
...
-- [Fix] isKillerChar: เช็ค weapon ทุก descendant
local function isKillerChar(char)
    ...
    -- fallback: เช็ค direct child "Weapon" (ชื่อตรงๆ)
...
local function hookPlayer_AP(plr)
    ...
    local function onChar(char)
        Hooked_AP[char] = nil
        task.wait(0.5)
        if isKillerChar(plr.Character or char) then
            hookChar_AP(char)
        end
    end
    if plr.Character then onChar(plr.Character) end
    plr.CharacterAdded:Connect(onChar)
    -- watch weapon added later
...
SurTab:Toggle({
    Title    = "Auto Parry",
    Desc     = "Parry killer attacks automatically BEFORE the hit lands",
    Value    = _G.AutoParry,
    Callback = function(v)
        _G.AutoParry = v
        Config:Set("autoparry", v); Config:Save()
        WindUI:Notify({ Title="Auto Parry (BETA)", Content=v and "Enabled" or "Disabled", Duration=3, Icon=v and "shield" or "shield-off" })
    end
})
SurTab:Dropdown({
    Title    = "Parry Mode",
...
Workspace.DescendantAdded:Connect(function()
    if not _invalidateScheduled then
        ...
        task.delay(1, function() _invalidateGenCache(); _invalidateScheduled = false end)
    end
end)
...
-- [Fix] removeESP: ป้องกัน error จาก destroyed instance
...
local function createESP(obj, baseColor)
    ...
    local function makeLabel(ypos, size)
        ...
        lbl.Size                   = UDim2.new(1,0, size or 0.33, 0)
...
-- [Fix] setObjectLabels: ใช้ pcall ป้องกัน error จาก destroyed
local function setObjectLabels(data, col, nameText, showName, hpText, showHp, distText, showDist)
    ...
    pcall(function()
        ...
        -- reposition based on what's visible
        local row = 0
        if data.nameLabel.Visible then
            data.nameLabel.Position = UDim2.new(0,0, row*0.33, 0); row += 1
        end
        if data.hpLabel.Visible then
            data.hpLabel.Position = UDim2.new(0,0, row*0.33, 0); row += 1
        end
        if data.distLabel.Visible then
            data.distLabel.Position = UDim2.new(0,0, row*0.33, 0)
        end
    end)
end
...
local function updateESP()
    ...
    -- [Fix] ─── PLAYER ESP (survivor + killer) ────────────────────────────────────
    for _, player in ipairs(Players:GetPlayers()) do
        if player == LocalPlayer then continue end
        local pChar = player.Character
        -- [Fix] skip lobby + check parent valid
        ...
        -- [Fix] ใช้ isKillerChar ที่ตรวจ deep
        ...
        -- [Fix] setObjectLabels สำหรับ player ด้วย
        local data = espObjects[pChar]
        if data then
    ...
    for _, desc in ipairs(Workspace:GetDescendants()) do
        ...
        local ok2, isValid2 = pcall(function() return desc.Parent ~= nil end)
        if not ok2 or not isValid2 then continue end
        ...
        if n == "Generator" then
            ...
            if espGenerator then
                ...
                if data and part then
                    local nameText = nil
                    if ShowName and ShowPercent then nameText = n.." | "..math.floor(progress*100).."%"
                    elseif ShowName then nameText = n end
                    local distText = ShowDistance and ("[ "..dist.." MM ]") or nil
        ...
        elseif n == "Gate" then
            ...
            for _, plr in ipairs(Players:GetPlayers()) do
                if plr.Character and desc:IsDescendantOf(plr.Character) then insidePlayer = true; break end
            end
    ...
    -- [Fix] Clean up stale espObjects (destroyed instances)
    for obj in pairs(espObjects) do
        local ok3, valid3 = pcall(function() return obj and obj.Parent ~= nil end)
        if not ok3 or not valid3 then removeESP(obj) end
    end
end
...
local function CreateCrosshair()
    ...
    f.Name = "Crosshair"; f.Size = UDim2.new(0,5,0,5); f.AnchorPoint = Vector2.new(0.5,0.5)
...
-- [Fix] Full Bright / No Fog — restore on load + fixed toggle logic
...
local function startFullBright()
    ...
    _fullBrightConn = RunService.RenderStepped:Connect(function()
        if not fullBrightEnabled then
            _fullBrightConn:Disconnect(); _fullBrightConn = nil; return
        end
...
local function startNoFog()
    ...
    _noFogConn = RunService.RenderStepped:Connect(function()
        if not noFogEnabled then
            _noFogConn:Disconnect(); _noFogConn = nil; return
        end
...
local GEN = {
    repairPoint = nil, repairModel = nil,
    cancelDB = false, skillDB = false,
    lastPos = nil, MOVE_THRESH = 0.9,
}
...
SurTab:Button({
    Title = "Fling Killer (Spam if doesn't fling)", Desc = "Attempts to fling the killer away using physics",
    Callback = function()
        local Targets = {}
        for _, plr in ipairs(Players:GetPlayers()) do
...
SurTab:Button({
    Title="Invisible (Not Visual)", Desc="Makes your character invisible to others",
...
SurTab:Button({
    Title="Self UnHook (Not 100%)", Desc="Attempts to free yourself from hooks",
...
killerTab:Toggle({
    Title="Enable Aimbot Charge (The Veil)", Desc="Charge-based aimbot for The Veil", Value=false,
    Callback=function(state)
...
killerTab:Toggle({ Title="Enable Aimbot (Toggle GUI)", Desc="Shows mobile toggle button for aimbot", Value=false,
    Callback=function(v) DYHUB_AimbotToggleGUIVisible=v; if DYHUB_mobileButton then DYHUB_mobileButton.Visible=v end end })
killerTab:Toggle({ Title="Enable Aimbot Charge (Toggle GUI)", Desc="Shows mobile toggle for aimbot charge", Value=false,
    Callback=function(v) DYHUB_Aimbot28ToggleGUIVisible=v; if DYHUB_mobileButton28 then DYHUB_mobileButton28.Visible=v end end })
killerTab:Toggle({ Title="Custom Position Drag (Toggle GUI)", Desc="Allows dragging the mobile aimbot buttons", Value=false,
...
local function DYHUB_GetPitchByDistance(d)
    for _,v in ipairs(_pitchTable) do if d<v[1] then return v[2] end end; return 23.3
end
...
local function DYHUB_EnsureGUIFolder()
    if not DYHUB_guiFolder or not DYHUB_guiFolder.Parent then
        ...
        DYHUB_guiFolder.Name="DYHUB_AimbotGUI"; DYHUB_guiFolder.ResetOnSpawn=false; DYHUB_guiFolder.Parent=PlayerGui
    end
end
local function DYHUB_CreateMobileButtons()
    ...
    local function makeBtn(text,pos,isEnabled)
        ...
        btn.Size=UDim2.new(0,90,0,90); btn.Position=pos; btn.AnchorPoint=Vector2.new(1,1)
        ...
        if v then
            task.spawn(function()
                while Stalker do
                    ...
                    local root=char and char:FindFirstChild("HumanoidRootPart")
                    if not root then continue end
                    if not isKillerChar(char) then continue end
...
killerTab:Toggle({
    Title="Remove Palletwrong (All)", Desc="Removes all Palletwrong objects", Value=destroyPalletwrong,
    Callback=function(v)
...
killerTab:Button({
    Title="Fix Cam (3rd Person Camera)", Desc="Changes the camera to third person view",
    Callback=function()
        local character=LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
...
PlayerTab:Toggle({
    Title="Enable Speed", Desc="Adjusts your character movement speed", Value=false,
    Callback=function(v)
        speedEnabled=v
...
setreadonly(mt,true)
PlayerTab:Toggle({ Title="No Fall (Beta)", Desc="Prevents movement slowdown after falling", Value=false,
...
local SelectedPlace
TeleportTab:Dropdown({ Title="Select Place", Values={"Lobby","Game"},
...
local GenDropdown=TeleportTab:Dropdown({
    ...
    Values=(function()
        local t={}; for _,g in ipairs(generatorList) do table.insert(t,g.Name) end; return t
    end)(),
    Callback=function(v)
        for _,g in ipairs(generatorList) do if g.Name==v then GenTarget=g.Object end end
    end
})
...
TeleportTab:Button({ Title="Refresh Generator", Desc="Updates generator list",
    Callback=function()
        ...
        local t={}; for _,g in ipairs(generatorList) do table.insert(t,g.Name) end; GenDropdown:Update(t)
    end })
...
TeleportTab:Button({ Title="Refresh All", Desc="Updates all dropdowns",
    Callback=function()
        ...
        GenTarget=nil; _invalidateGenCache()
...
Main3:Button({
    Title="Save Config (NOW)", Desc="Saves all current settings immediately.",
    Callback=function()
        Config:Save()
...
Main3:Toggle({
    Title="Auto Save Config", Value=AutoSaveEnabled, Desc="Automatically saves config at set interval.",
    Callback=function(state)
...
Main3:Button({
    Title="Serverhop", Desc="Teleports you to a different random server.",
    Callback=function()
        local servers={}
...
local function LoadDiscordInfo()
    local success,result=pcall(function()
        return HttpService:JSONDecode(ui.Creator.Request({ Url=DiscordAPI, Method="GET",
            Headers={ ["User-Agent"]="RobloxBot/1.0", ["Accept"]="application/json" } }).Body)
    end)
...
Info:Paragraph({ Title="Social", Desc="Copy link social media for follow!",
    Image="rbxassetid://104487529937663", ImageSize=30,
    Buttons={{ Icon="copy", Title="Copy Link", Callback=function() setclipboard("https://guns.lol/DYHUB") end }} })
Info:Paragraph({ Title="Discord", Desc="Join our discord for more scripts!",
    Image="rbxassetid://104487529937663", ImageSize=30,
