...
function TeleportResetWave()
    local now = tick()
    if now - (ResetWaveLastTeleportAt or 0) < 1.2 then return false end
    ...
    task.delay(0.65, function()
        ...
        if ResetWaveEnabled then
            FarmForceRetarget = false
            _interruptSignal = false
            if AutoFarmEnabled and StartFarmLoop then
                StartFarmLoop()
            end
        end
    end)
    ...
    if ok then
        CombatDebug("ResetWave", "Teleported at wave " .. tostring(ResetWaveValue), 2, false)
    end
    ...
    task.spawn(function()
        while ResetWaveEnabled do
            if IsMiscFarmAllowed() then
                ...
                local targetWave = tonumber(ResetWaveValue) or 10

                if currentWave ~= nil then
                    if currentWave >= targetWave then
                        if ResetWaveLastTriggeredWave ~= currentWave then
