...
GlobalTables = {
    ...
    Gamepassts = {},
    GachaSpin = { "1Spin", "10Spins", "100Spins", "1SpinLucky", "10SpinLucky" },
    UseItems = { "Presents" },
}
...
local _openVoteLoopRunning = false
local _openVoteCycleBusy   = false
local _lastOpenVoteFrame   = nil

local AUTO_VOTE_FIRE_COUNT = 10
local AUTO_VOTE_FIRE_GAP   = 0.03
local OPEN_VOTE_SCAN_DELAY = 0.10
...
local function GetOpenVoteFrame()
    local playerGui = LocalPlayer:FindFirstChild("PlayerGui") or pg
...
local function IsOpenVoteVisible()
    local frame = GetOpenVoteFrame()
    _lastOpenVoteFrame = frame
    return frame and frame.Visible == true
end

local function SetOpenVoteVisible(state)
    local frame = GetOpenVoteFrame() or _lastOpenVoteFrame
    if frame then
        pcall(function()
            frame.Visible = state
        end)
...
local function FireVote_InGame(count)
    if not AutoVoteValue then return end
    count = tonumber(count) or 1

    for i = 1, count do
        if not AutoVoteinGameEnabled then break end
        pcall(function()
            ReplicatedStorage.Vote:FireServer(AutoVoteValue)
        end)
        if i < count then task.wait(AUTO_VOTE_FIRE_GAP) end
    end
    --print("[DYHUB] AutoVote IG fired: " .. tostring(AutoVoteValue) .. " x" .. tostring(count))
end

local function RunOpenVoteCycle()
    if _openVoteCycleBusy then return end
    if not AutoVoteinGameEnabled then return end

    _openVoteCycleBusy = true

    FireVote_InGame(AUTO_VOTE_FIRE_COUNT)
    SetOpenVoteVisible(false)

    -- Sync: Auto Vote + Auto Start
    -- เมื่อ Vote ยิงครบแล้ว ให้ปิด UI แล้วเริ่มเกมต่อทันที
    if AutoVoteinGameEnabled and AutoStartEnabled then
        FireGetReady(0.15)
    end

    task.wait(0.2)
    _openVoteCycleBusy = false
end

local function StartOpenVoteWatcher()
    if _openVoteLoopRunning then return end
    _openVoteLoopRunning = true

    task.spawn(function()
        while AutoVoteinGameEnabled do
            if IsOpenVoteVisible() and not _openVoteCycleBusy then
                RunOpenVoteCycle()

                -- รอจนกว่า UI จะปิดจริงก่อน ถึงจะรับรอบใหม่
                repeat
                    task.wait(OPEN_VOTE_SCAN_DELAY)
                until (not AutoVoteinGameEnabled) or (not IsOpenVoteVisible())
            end

            task.wait(OPEN_VOTE_SCAN_DELAY)
        end

        _openVoteLoopRunning = false
        _openVoteCycleBusy = false
    end)
end
...
local function SetupAutoStartOnly(enabled)
    ...
    AutoStartConnection = LocalPlayer.CharacterAdded:Connect(function()
        _openVoteCycleBusy = false
        task.wait(1)
        if AutoStartEnabled and not AutoVoteEnabled then task.spawn(function() FireGetReady() end) end
    end)
end
...
local function StartAutoStart()
    ...
    if AutoVoteinGameEnabled then StartOpenVoteWatcher() end
end
...
local _cameraModeLoopRunning = false

local function GetCameraSubjectByMode()
    local char = LocalPlayer.Character
    if not char then return nil end

    if CameraMode == "Classic" then
        return char:FindFirstChild("Head") or char:FindFirstChildOfClass("Humanoid") or char:FindFirstChild("HumanoidRootPart")
    end

    return char:FindFirstChild("HumanoidRootPart") or char:FindFirstChildOfClass("Humanoid") or char:FindFirstChild("Head")
end
...
local function ApplyCameraMode()
    pcall(function()
        ...
        local subject = GetCameraSubjectByMode()
        if subject then
...
local function StartCameraModeLoop()
    if _cameraModeLoopRunning then return end
    _cameraModeLoopRunning = true
    task.spawn(function()
        while true do
            ApplyCameraMode()
            task.wait(1)
...
-- ====================== ISOLATED LATE UI / SHOP / STARTUP SECTION ======================
-- Wrapped in its own function to avoid Roblox/Luau local-register limit (200 locals).
(function()
...
-- ====================== UI: AUTO GACHA / USE ITEM ======================
Main5:Section({ Title = "Auto Gacha", Icon = "gift" })

local AutoGachaCharacterEnabled = Config:Get("AutoGachaCharacterEnabled", false)
local AutoGachaCharacterValue   = Config:Get("AutoGachaCharacterValue", "1Spin")
local AutoGachaSkinEnabled      = Config:Get("AutoGachaSkinEnabled", false)
local AutoGachaSkinValue        = Config:Get("AutoGachaSkinValue", "1Spin")
local AutoGachaDelay            = Config:Get("AutoGachaDelay", 1)

local AutoUseItemEnabled        = Config:Get("AutoUseItemEnabled", false)
local AutoUseItemValue          = Config:Get("AutoUseItemValue", "Presents")
local AutoUseItemDelay          = Config:Get("AutoUseItemDelay", 1)

local _autoGachaCharacterRunning = false
local _autoGachaSkinRunning      = false
local _autoUseItemRunning        = false

local function FireGachaRemote(remoteName, spinValue)
    local remote = ReplicatedStorage:FindFirstChild(remoteName)
    if not remote or not spinValue then return false end

    local ok = pcall(function()
        remote:FireServer(spinValue)
    end)
...
local function StartAutoGachaCharacter()
    if _autoGachaCharacterRunning then return end
    _autoGachaCharacterRunning = true

    task.spawn(function()
        while AutoGachaCharacterEnabled do
            FireGachaRemote("GachaCharacter", AutoGachaCharacterValue)
            task.wait(math.max(0.25, tonumber(AutoGachaDelay) or 1))
        end

        _autoGachaCharacterRunning = false
    end)
end

local function StartAutoGachaSkin()
    if _autoGachaSkinRunning then return end
    _autoGachaSkinRunning = true

    task.spawn(function()
        while AutoGachaSkinEnabled do
            FireGachaRemote("GachaSkins", AutoGachaSkinValue)
            task.wait(math.max(0.25, tonumber(AutoGachaDelay) or 1))
        end

        _autoGachaSkinRunning = false
    end)
end

local function FireUseItem()
    if AutoUseItemValue == "Presents" then
...
local function StartAutoUseItem()
    if _autoUseItemRunning then return end
    _autoUseItemRunning = true

    task.spawn(function()
        while AutoUseItemEnabled do
            FireUseItem()
            task.wait(math.max(0.25, tonumber(AutoUseItemDelay) or 1))
        end

        _autoUseItemRunning = false
    end)
end

GachaCharacterDropdown = Main5:Dropdown({
    Title = "Auto Gacha Character",
    Values = GlobalTables.GachaSpin,
    Multi = false,
    Value = AutoGachaCharacterValue,
    Callback = function(value)
        AutoGachaCharacterValue = value
        Config:Set("AutoGachaCharacterValue", value)
        Config:Save()
    end
})
...
GachaSkinDropdown = Main5:Dropdown({
    ...
    Values = GlobalTables.GachaSpin,
    Multi = false,
    Value = AutoGachaSkinValue,
    Callback = function(value)
        AutoGachaSkinValue = value
        Config:Set("AutoGachaSkinValue", value)
        Config:Save()
    end
})
...
Main5:Section({ Title = "Auto Use Item", Icon = "package" })

UseItemDropdown = Main5:Dropdown({
    Title = "Auto Use Item",
    Values = GlobalTables.UseItems,
    Multi = false,
    Value = AutoUseItemValue,
    Callback = function(value)
        AutoUseItemValue = value
        Config:Set("AutoUseItemValue", value)
        Config:Save()
    end
})
...
CameraModeDropdown = Main3:Dropdown({
    Title = "Camera",
...
local function StartAntiAFKSystem()
    ...
    if not _antiAFKConnection then
        _antiAFKConnection = game.Players.LocalPlayer.Idled:Connect(function()
            if AntiAFK then
...
local antiafk = Main3:Toggle({
    ...
    Callback = function(enabled)
        ...
        if enabled then StartAntiAFKSystem() end
    end
})
...
if AntiAFK then StartAntiAFKSystem() end
...
-- Re-apply saved Misc options on script load.
-- If Sync Farm Only is disabled, these can run even without Auto Farm.
HandleMiscOptions(MiscOptions)
