        ...
        if IsValidMob(mob) then
            ...
            if humanoid and mobRoot then
                ...
                if health > highestHealth then
                    ...
                    highestMob = mob
                elseif health == highestHealth and distance < nearestDistance then
                    nearestDistance = distance
                    highestMob = mob
                end
            end
...
-- ====================== PRIORITY SYSTEM ======================
-- Helicopter = Highest Priority
-- Highest HP Monster = Second Priority
-- Lowest HP Monster = Last Priority
-- Dynamic target switching enabled
...
local function GetMobPriorityValue(mob)
    if not mob or not mob.Parent then
        return -1
    end

    if mob.Name == "Helicopter" then
        return 999999999
    end
...
local function ShouldSwitchTarget(currentMob)
    if not currentMob or not currentMob.Parent then
        return true
    end

    local bestMob = GetPriorityMob()

    if not bestMob or bestMob == currentMob then
        return false
    end

    local currentPriority = GetMobPriorityValue(currentMob)
    local bestPriority = GetMobPriorityValue(bestMob)
        ...
        while AutoFarmEnabled do
            ...
            if mob then
                ...
                if mobType == "GiantST" and extraData then
                    ...
                    until IsMobDead(mob) or not mob.Parent or not AutoFarmEnabled or ShouldSwitchTarget(mob)
                    ...
                    if SafeModeEnabled and GetPlayerHealthPercent() < SafeValue then
                        ...
                        repeat
                            ...
                            if ShouldSwitchTarget(mob) then
                                break
                            end
