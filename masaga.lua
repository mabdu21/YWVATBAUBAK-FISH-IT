...
DYHUB_VoteState = DYHUB_VoteState or {}
DYHUB_VoteState.RunSerial = 0
DYHUB_VoteState.LastCharacter = nil
DYHUB_VoteState.FiredThisLife = false

function DYHUB_GetOpenVoteUIObject()
...
function DYHUB_IsOpenVoteUIVisible()
    local openUI = DYHUB_GetOpenVoteUIObject()
...
function DYHUB_FireVote_InGameBurst()
...
function DYHUB_ArmAutoVoteInGameForCurrentLife()
    if not AutoVoteinGameEnabled then return end

    local char = LocalPlayer.Character
    if not char then return end

    if DYHUB_VoteState.LastCharacter ~= char then
        DYHUB_VoteState.LastCharacter = char
        DYHUB_VoteState.FiredThisLife = false
    end

    if DYHUB_VoteState.FiredThisLife then return end

    DYHUB_VoteState.RunSerial = (DYHUB_VoteState.RunSerial or 0) + 1
    local mySerial = DYHUB_VoteState.RunSerial

    task.spawn(function()
        while mySerial == DYHUB_VoteState.RunSerial and AutoVoteinGameEnabled and LocalPlayer.Character == char and not DYHUB_VoteState.FiredThisLife do
            if DYHUB_IsOpenVoteUIVisible() then
                DYHUB_VoteState.FiredThisLife = true
                DYHUB_FireVote_InGameBurst()

                -- Auto Vote + Auto Start: vote x10 -> wait 1s -> ready remote x1
                if AutoStartEnabled then
                    task.wait(1)
                    if mySerial == DYHUB_VoteState.RunSerial and AutoVoteinGameEnabled and AutoStartEnabled and LocalPlayer.Character == char then
                        FireGetReady(0)
                    end
                end
                break
...
local function RefreshVoteAndStartSetup()
    ...
    if AutoVoteinGameEnabled then
        DYHUB_ArmAutoVoteInGameForCurrentLife()
    end
end
...
local function SetupAutoVote_InGame(enabled)
    ...
    DYHUB_VoteState.RunSerial = (DYHUB_VoteState.RunSerial or 0) + 1
    DYHUB_VoteState.LastCharacter = LocalPlayer.Character
    DYHUB_VoteState.FiredThisLife = false
...
local function StartAutoStart()
    ...
    if AutoVoteinGameEnabled then DYHUB_ArmAutoVoteInGameForCurrentLife() end
end
