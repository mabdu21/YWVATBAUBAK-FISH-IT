...
RunService.Heartbeat:Connect(function(dt)
    ...
    local char = LocalPlayer.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    local hum  = char and char:FindFirstFind("Humanoid")
    ...
    if (moved > GEN.MOVE_THRESH or hum.MoveDirection.Magnitude > 0.05) and not GEN.cancelDB then
        cancelRepair()
    end
end)
...
SurTab:Toggle({
    ...
    Callback = function(v)
        ...
        if v then AutoSkillNeutral = false; notify("Auto Skill Perfect", "Active."); startSkillLoop("perfect")
...
SurTab:Toggle({
    ...
    Callback = function(v)
        ...
        if v then AutoSkillPerfect = false; notify("Auto Skill Neutral", "Active."); startSkillLoop("neutral")
        elseif _skillThread then task.cancel(_skillThread); _skillThread = nil end
    end
})
