--[[
    Hotfix v032.51:
      * fixed Information tab nil error (Info tab was created as Tab)
      * moved premium/discord HTTP checks to background to reduce first-run freeze
      * cached WindUI source in memory for faster re-execute
]]
...
do
    local windSource = ENV.__DYHUB_WINDUI_SOURCE
    if type(windSource) ~= "string" or #windSource < 100 then
        windSource = game:HttpGet("https://github.com/Footagesus/WindUI/releases/latest/download/main.lua")
        ENV.__DYHUB_WINDUI_SOURCE = windSource
    end
...
local userversion = FreeVersion
task.defer(function()
    task.wait(0.25)
    local checkedVersion = checkVersion(LocalPlayer.Name)
    userversion = checkedVersion or FreeVersion
    if checkedVersion and checkedVersion ~= FreeVersion then
        notify("Version Check", "Account detected: " .. tostring(checkedVersion), 2.5, "badge-check")
    end
end)
...
-- Create Discord widgets immediately so they stay inside the Discord Information section.
-- Only the text gets updated later; async loading will not append items to the bottom anymore.
local DiscordInfoCard = Info:Paragraph({
    Title = "Discord Server",
    Desc = "Loading Discord statistics...",
    Image = "rbxassetid://104487529937663",
    ImageSize = 30,
})

local function setParagraphTitle(paragraph, title)
    pcall(function()
...
local function setParagraphDesc(paragraph, desc)
    pcall(function()
        if paragraph and paragraph.SetDesc then
            paragraph:SetDesc(tostring(desc))
        end
    end)
end
...
local function fetchDiscordInfo()
    local success, result = pcall(function()
        ...
        -- Fallback for executors that only expose HttpService requests.
        local response = ui.Creator.Request({ Url = DiscordAPI, Method = "GET" })
        if response and response.Body then
            return game:GetService("HttpService"):JSONDecode(response.Body)
        end
        return nil
...
local function applyDiscordInfo(result)
    if result and result.guild then
        setParagraphTitle(DiscordInfoCard, result.guild.name or "Discord Server")
        setParagraphDesc(DiscordInfoCard,
            ' <font color="#52525b">●</font> Member Count : ' .. tostring(result.approximate_member_count or "?") ..
            '\n <font color="#16a34a">●</font> Online Count : ' .. tostring(result.approximate_presence_count or "?")
        )
    else
        setParagraphTitle(DiscordInfoCard, "Discord Server")
        setParagraphDesc(DiscordInfoCard, "Unable to load Discord information right now.")
    end
end

local function LoadDiscordInfo()
    applyDiscordInfo(fetchDiscordInfo())
end

Info:Button({
    Title = "Update Info",
    Desc = "Refreshes Discord member and online statistics.",
    Callback = function()
        setParagraphDesc(DiscordInfoCard, "Refreshing Discord statistics...")
        task.spawn(function()
            local result = fetchDiscordInfo()
            applyDiscordInfo(result)
            if result and result.guild then
...
Info:Button({
    ...
    Desc = "Copies the DYHUB Discord invite link.",
    Callback = function()
